...
The `codex/docker` branch adds three commits for project source selection,
Statsig registration compatibility, and Docker packaging with upstream
automation. The commits remain a linear stack rebased onto upstream `main`.

## Automated updates

The Rebase downstream and publish workflow checks upstream every six hours and
can also be started manually. Scheduled runs continue only when upstream has
changed. Manual runs always build and publish the current downstream branch.

When upstream changes, the workflow:
...
`codex/archive/YYYY-MM-DD_HHMMSSZ`. These timestamped UTC branches keep the
previous downstream stack available after rebases.

## Optional passwordless sudo

The image entrypoint starts as root to prepare the configured runtime identity,
then launches codex-web as an unprivileged user. Passwordless sudo is disabled
by default. The container entrypoint accepts
...
The default `compose.yml` publishes only Caddy on port 8214 across the host's
network interfaces; codex-web remains on the private Compose network.
`compose.direct.yml` bypasses Caddy and publishes codex-web only on
`127.0.0.1:8214`.
